...
MAX_STATUS_CHECKS_PER_RUN = int(os.getenv("MAX_STATUS_CHECKS_PER_RUN", "20"))
SEARCH_DELAY_MIN_SECONDS = float(os.getenv("SEARCH_DELAY_MIN_SECONDS", "2"))
SEARCH_DELAY_MAX_SECONDS = float(os.getenv("SEARCH_DELAY_MAX_SECONDS", "4"))
...
def main():
    ...
    selected, cursors = select_query_groups(groups, MAX_SEARCHES_PER_RUN, cursors)
    try:
        previous_failed = json.loads(state.get("failed_queries", "[]"))
    except (TypeError, ValueError):
        previous_failed = []
    retry_keys = {normalize_match_text(query) for query in previous_failed[:MAX_SEARCHES_PER_RUN]}
    retry_groups = [group for group in groups if group["key"] in retry_keys]
    selected = (retry_groups + [group for group in selected if group["key"] not in retry_keys])[:MAX_SEARCHES_PER_RUN]
    save_state(state_ws, state_rows, "search_cursors", json.dumps(cursors, ensure_ascii=False))
    ...
    for index, group in enumerate(selected):
        ...
        try:
            results = yahoo_client.search(group["query"], timeout=HTTP_TIMEOUT_SECONDS, retries=HTTP_RETRIES)
            ...
            failed_queries.append(group["query"])
    save_state(state_ws, state_rows, "failed_queries", json.dumps(failed_queries, ensure_ascii=False))
    ...
    for item in found.values():
        try:
            ...
            if existing:
                update_record(items_ws, existing[0], record)
                continue
            append_record(items_ws, record)
            append_record(history_ws, {
    ...
    save_state(state_ws, state_rows, "status_cursor", str(next_status_cursor))
    ...
    for index, (row_number, row) in enumerate(checks):
        ...
        update_record(items_ws, row_number, updated)
        sold_at = now_jst()
        try:
        ...
        append_record(sold_ws, sold_record)
